'use client'

import { motion } from 'framer-motion'

/* ← personaliza tus datos de contacto */
const email = 'tu@email.com'

const socialLinks = [
  {
    label: 'GitHub',
    href: 'https://github.com/tu-usuario',
    icon: (
      <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0 0 22 12.017C22 6.484 17.522 2 12 2Z" />
      </svg>
    ),
  },
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/in/tu-usuario',
    icon: (
      <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286ZM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065Zm1.782 13.019H3.555V9h3.564v11.452ZM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003Z" />
      </svg>
    ),
  },
  {
    label: 'Twitter / X',
    href: 'https://twitter.com/tu-usuario',
    icon: (
      <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
]

const fadeUp = {
  hidden: { opacity: 0, y: 24 },
  show:   { opacity: 1, y: 0,  transition: { duration: 0.6, ease: 'easeOut' } },
}

export default function Contact() {
  return (
    <section id="contact" className="py-28 px-6" style={{ background: 'var(--surface)' }}>
      <div className="max-w-2xl mx-auto text-center">

        <motion.div
          variants={{ hidden: {}, show: { transition: { staggerChildren: 0.12 } } }}
          initial="hidden"
          whileInView="show"
          viewport={{ once: true, margin: '-80px' }}
        >
          <motion.p variants={fadeUp} className="font-mono text-sm mb-3" style={{ color: 'var(--accent)' }}>
            05. Contacto
          </motion.p>

          <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold mb-4">
            ¿Hablamos?
          </motion.h2>

          <motion.p variants={fadeUp} className="mb-10 leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            {/* ← personaliza el mensaje */}
            Estoy abierto a nuevas oportunidades, proyectos freelance o simplemente
            una charla sobre tecnología. Mi bandeja de entrada siempre está abierta.
          </motion.p>

          {/* Email CTA */}
          <motion.a
            href={`mailto:${email}`}
            className="inline-block rounded-lg px-8 py-4 text-base font-semibold text-white mb-12"
            style={{ background: 'var(--gradient)', boxShadow: `0 0 24px var(--accent-glow)` }}
            variants={fadeUp}
            whileHover={{ scale: 1.05, boxShadow: '0 0 40px var(--accent-glow)' }}
            whileTap={{ scale: 0.97 }}
          >
            Enviar email
          </motion.a>

          {/* Social links */}
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-6">
            {socialLinks.map((link) => (
              <motion.a
                key={link.label}
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={link.label}
                className="rounded-xl p-3"
                style={{
                  background: 'var(--surface-raised)',
                  color: 'var(--text-muted)',
                  border: '1px solid var(--border)',
                }}
                whileHover={{
                  scale: 1.12,
                  color: 'var(--accent)',
                  borderColor: 'var(--accent)',
                  background: 'var(--accent-glow)',
                }}
                whileTap={{ scale: 0.95 }}
                transition={{ duration: 0.18 }}
              >
                {link.icon}
              </motion.a>
            ))}
          </motion.div>

        </motion.div>
      </div>

      {/* Footer */}
      <motion.p
        className="mt-20 text-center text-xs"
        style={{ color: 'var(--text-subtle)' }}
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6, delay: 0.3 }}
      >
        {/* ← cambia tu nombre */}
        Diseñado y construido por <span className="gradient-text font-medium">Tu Nombre</span>
      </motion.p>
    </section>
  )
}
